  ...
  const handleReaction = (msgId: string, emoji: string) => {
    ...
    setMessages((prev) => {
      ...
      if (existingIdx >= 0) {
        ...
        if (r.reacted) { r.count -= 1; r.reacted = false; } else { r.count += 1; r.reacted = true; }
  ...
  return (
    <AppLayout title="Messages">
      <div className="flex h-[calc(100vh-7.5rem)] lg:h-[calc(100vh-4rem)]">
        {/* Left panel */}
        <div className={`w-full lg:max-w-sm border-r border-border/50 flex flex-col bg-card/30 ${showChat ? "hidden lg:flex" : "flex"}`}>
                    ...
                    >
                      <div className="relative">
                        <Avatar className="h-10 w-10">
                          <AvatarFallback className="bg-primary/15 text-primary text-xs font-semibold">{convo.initials}</AvatarFallback>
                        </Avatar>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {convo.unread > 0 && (
                            <span className="ml-2 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground tabular-nums">{convo.unread}</span>
                          )}
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <Avatar className="h-10 w-10 shrink-0">
                          <AvatarFallback className="bg-primary/15 text-primary text-xs font-semibold">{req.from.initials}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                          ...
                          {req.status === "pending" ? (
                            <div className="mt-3 flex gap-2">
                              <Button size="sm" className="h-8 gap-1.5 text-xs" onClick={() => handleAcceptIntro(req.id)}>
          ...
          {currentConvo ? (
            <>
              ...
              <div className="flex items-center gap-3 border-b border-border/50 px-4 lg:px-6 py-4 shrink-0 bg-card/30">
                ...
                <div className="relative">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback className="bg-primary/15 text-primary text-xs font-semibold">{currentConvo.initials}</AvatarFallback>
                  </Avatar>
              ...
              <div className="border-t border-border/50 p-4 bg-card/30">
                <div className="flex items-center gap-2 max-w-3xl mx-auto">
                  ...
                  />
                  <Button size="icon" onClick={handleSendMessage} disabled={!messageInput.trim()} className="shrink-0">
            ...
            <div className="flex-1 flex items-center justify-center">
              ...
              >
                ...
                <h3 className="font-display text-lg font-semibold text-foreground mb-1">Your messages</h3>
